...
function isImageOnlyAccept(accept: Accept | undefined): boolean {
    ...
    return mimeTypes.length > 0 && mimeTypes.every((type) => type.startsWith('image/'));
}

// Helper function to get dropzone icon
function getDropzoneIcon(accept: Accept | undefined, emptyIcon: React.ReactNode | undefined): React.ReactNode {
    if (emptyIcon) {
        return emptyIcon;
    }
...
function getAcceptDescription(accept: Accept | undefined): string {
    ...
    for (const extensions of Object.values(accept)) {
        if (extensions && extensions.length > 0) {
            allExtensions.push(...extensions.map((ext) => ext.toUpperCase().replace('.', '')));
        }
    }
...
function getDropzoneText(
    ...
    const title =
        emptyTitle ||
        (isImageOnly ? (maxFiles === 1 ? 'Drop your image here' : 'Drop your images here') : maxFiles === 1 ? 'Drop file here' : 'Drop files here');
    ...
    const handleRejections = (fileRejections: FileRejection[]) => {
        ...
        for (const rejection of fileRejections) {
            ...
            for (const error of errors) {
                const formatted = formatRejectionError(file, error.code, error.message, maxSize, maxFiles);
                errorMessages.push({
                    type: 'validation_error',
        ...
        >
            ...
            <div className="flex flex-col items-center justify-center px-4 py-3 text-center">
                ...
                >
                    ...
                    Select {isImageOnly ? (maxFiles === 1 ? 'image' : 'images') : maxFiles === 1 ? 'file' : 'files'}
                </Button>
            </div>

            {fileCountText && <div className="absolute right-3 bottom-4 text-xs text-muted-foreground">{fileCountText}</div>}
        </div>
    );
}
