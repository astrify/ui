import type { SignedUrlResponse, UploadError, UploadLib } from '@astrify/react-s3-upload';

/**
 * Helper to simulate upload progress over time
 */
const simulateProgress = async (onProgress?: (progress: number) => void, duration = 2000): Promise<void> => {
    const steps = 20;
...
export const createUploadSuccessFake = (): UploadLib => ({
    ...
    requestBatchSignedUrls: async (files: Array<{ file: File; sha256: string }>): Promise<SignedUrlResponse[]> => {
 ...
 */
export const createUploadFailureFake = (failurePattern = DEFAULT_FAILURE_PATTERN): UploadLib => {
    let retryCount = 0;
...
export const createFastUploadFake = (): UploadLib => ({
    ...
    requestBatchSignedUrls: (files: Array<{ file: File; sha256: string }>): Promise<SignedUrlResponse[]> => {
 ...
 */
export const createFastUploadErrorFake = (errorType: 'network_error' | 'validation_error' | 'duplicate_file' = 'network_error'): UploadLib => ({
    calculateSHA256: (file: File): Promise<string> => {
        return Promise.resolve(createMockHash(file));
    },
