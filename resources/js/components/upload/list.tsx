        ...
        if (file.status === 'pending' || file.status === 'uploading') {
            ...
            return (
                <div className="relative size-10 overflow-hidden rounded">
                    {/* Dimmed base image */}
                    <img src={previewUrl} alt={file.name} className="absolute inset-0 size-10 object-cover opacity-30" />
                    {/* Progressive reveal overlay */}
                    <div className="absolute inset-0 overflow-hidden" style={{ width: `${progress}%` }}>
                        <img src={previewUrl} alt={file.name} className="size-10 object-cover" />
                    </div>
                    {/* Optional: Add a subtle progress line */}
                    {file.status === 'uploading' && progress > 0 && progress < 100 && (
                        <div className="absolute inset-y-0 w-0.5 bg-primary/50" style={{ left: `${progress}%` }} />
                    )}
                </div>
            );
        }
